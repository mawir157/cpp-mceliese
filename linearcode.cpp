...
{
    mv_generator = M;
    prepend_identity(mv_generator);

    mv_check = M;
    mv_check = transpose(mv_check);
    append_identity(mv_check);

    mn_max_errors = (calc_minimum_weight() / 2) - 1;
    mm_syndromes = build_syn_table(mv_check, mn_max_errors);
}
...
{
    const size_t max_word = 1 << mn_code_width;
...
void LinearCode::print()
{
    ...
    std::cout << mv_generator.size() << std::endl;
    print_matrix(mv_generator, 2 * mn_code_width);
    std::cout << "Checking matrix:" << std::endl;
    std::cout << mv_check.size() << std::endl;
    print_matrix(mv_check, 2 * mn_code_width);
